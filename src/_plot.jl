function plot(fig, graph, name, W, H; radius = 2, rect_w = 1, rect_h = 6, root = :center, lw = 1.0, v_line_dist = 2.0, lstyle = :rectangular, bstyle = :rectangle, font_size = 8, text_pad = 1.0, text_loc = :top, highlight_edges = nothing)
    ...
    coords = GraphVisualization.compute_positions_grid(graph, W, H, fig.padding)
    ...
    elseif lstyle == :rectangular
        ...
        for e in edges
            ...
            set_line_width(fig.cairo_context, lw)
            if highlight_edges != nothing
                if highlight_edges[ectr]
                    set_source_rgb(fig.cairo_context, [1, 0, 0]...)
                end
            else
                set_source_rgb(fig.cairo_context, [0, 0, 1]...)
            end
